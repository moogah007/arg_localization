...
class citi_reports(models.TransientModel):
    ...
    def getallcodessl(self,invoice=None):
        ...
        return self.tax_line_ids.filtered(lambda r: (
            r.tax_id.tax_group_id.type == 'tax' and
            r.tax_id.tax_group_id.tax == 'vat') and
            r.tax_id.tax_group_id.afip_code in (1,2,3,5)).mapped('tax_id.id')
    ...
    def getbaseforcodet(self,invoice=None,vatcode=None):
        if invoice!=None:
            self = invoice
        return sum(self.tax_line_ids.filtered(lambda r: (
            r.tax_id.tax_group_id.type == 'tax' and
            r.tax_id.tax_group_id.tax == 'vat' and
            r.tax_id.tax_group_id.afip_code == vatcode and r.base>0)).mapped('base'))
    ...
    def Print_citi(self):
        ...
        for inv in invoices:
            if inv.journal_id.use_documents:
                ...
                total = converttolong(VATReport_wizard.def_sales_reports_reportvat.gettotprofit(inv))
                tstr = tstr + "{:0>15}".format(total)

                total = converttolong(VATReport_wizard.def_sales_reports_reportvat.gettotgrossincome2(inv))
                tstr = tstr + "{:0>15}".format(total)

                total = converttolong(VATReport_wizard.def_sales_reports_reportvat.gettotlocPercep(inv))
                tstr = tstr + "{:0>15}".format(total)
                ...
                total = converttolong(VATReport_wizard.def_sales_reports_reportvat.gettotothers(inv))
                tstr = tstr + "{:0>15}".format(total)
        ...
        for inv in invoices:
            if "{:0>3}".format(inv.journal_document_type_id.document_type_id.code) not in [
                ...
                if inv.fiscal_position_id.afip_code == False or \
                        inv.journal_document_type_id.document_type_id.code in ['195','196''197']:
                ...
                for code in vatcodes:
                    ...
                    if inv.journal_document_type_id.document_type_id.code in ['195','196''197']:
                        total = self.getbaseforcodet(inv, gARRAYVAT[code])
                    else:
        ...
        for inv in invoices:
            if inv.journal_id.use_documents:
                ...
                tstr_v_cbte += "{:0>5}".format(doc_nr[0])
                ...
                total = converttolong(VATReport_wizard.def_sales_reports_reportvat.gettotpercepincome(inv))
                tstr_v_cbte += "{:0>15}".format(total)
                ...
                if inv.journal_document_type_id.document_type_id.document_letter_id.name != 'E' and \
                        inv.journal_document_type_id.document_type_id.code not in ["60","63","64","82","81"] and not \
        ...
        for inv in invoices:
            if inv.journal_id.use_documents:
                if "{:0>3}".format(inv.journal_document_type_id.document_type_id.code) != '066' and \
                    ...
                    #print "-----------"
                    #print inv.document_number, inv.fiscal_position_id.afip_code
                    if inv.fiscal_position_id.afip_code == False:
                        vatcodes = self.getallcodes(inv)
                        #print "sin posicion"
                    else:
                    ...
                    for code in vatcodes:
                        ...
                        if inv.fiscal_position_id.afip_code == False:
                            ...
                            if inv.journal_document_type_id.document_type_id.document_letter_id.name not in ('E',"T"):
                                total = 0
                            else:
                                total = self.getbaseforcode(inv, gARRAYVAT[code])
                        ...
                        if inv.fiscal_position_id.afip_code == False:
                            ...
                            if inv.journal_document_type_id.document_type_id.document_letter_id.name != "T":
