...
class sire_report(models.TransientModel):
    ...
    wh_code = fields.Many2many('account.tax','l1', 'l2', 'tax_id',string='Cod Retencion',
    ...
    date_to = fields.Date(string='Date To', required=True,
        default=str(datetime.now() + relativedelta.relativedelta(months=+1, day=1, days=-1))[:10])
    percep_code = fields.Many2many('account.tax','l3','l4','tax_id',string='Cod Percepcion',
    ...
    def do_percepf(self,invoices,nc=False):
        ...
        for inv in invoices:
            for code in inv.tax_line_ids:
                if code.tax_id.id in list(self.percep_code._ids) and code.amount>0:
                    date = inv.date_invoice
                    if nc:
                        str1 += "2"
                        str1 += "{:0>12}".format(inv.document_number.replace('-','')[-8:])
                        str1 += inv.date_invoice[8:10] + '/' + inv.date_invoice[5:7] + '/' + inv.date_invoice[0:4]
                        t1 = MultiplybyRate(inv.currency_rate, inv.amount_total, inv.company_currency_id,
                                            inv.currency_id)
                        str1 += "{:0>16}".format(str(t1).replace('.',','))
                        str1 += "{:>16}".format(" ")

                        orig = self.env['account.invoice'].search([('id','=',inv.refund_invoice_id.id)])
                        if orig.document_type_id.internal_type == 'debit_note' and orig.mipymesf:
                            str1 += "11"
                        elif orig.document_type_id.internal_type == 'invoice' and orig.mipymesf:
                            str1 += "10"
                        elif orig.document_type_id.internal_type == 'invoice':
                            str1 += "01"
                        ...
                        if nc_orig:
                            str1 += "{:0>16}".format(nc_orig.document_number.replace('-',''))
                        ...
                        str1 += "{:0>16}".format(str(t1).replace('.',','))
                        t1 = "{:.2f}".format(code.amount / code.base * 100)
                        str1 += "{:0>5}".format(str(t1).replace('.',','))

                    else:
                        str1 += "2"
                        str1 += "{:0>3}".format(code.tax_id.sicore_norm.name)
                        str1 += inv.date_invoice[8:10] + '/' + inv.date_invoice[5:7] + '/' + inv.date_invoice[0:4]
                        if inv.document_type_id.internal_type == 'debit_note' and inv.mipymesf:
                            str1 += "11"
                        elif inv.document_type_id.internal_type == 'invoice' and inv.mipymesf:
                            str1 += "10"
                        elif inv.document_type_id.internal_type == 'invoice':
                            str1 += "01"
                        ...
                        str1 += inv.document_type_id.document_letter_id.name
                        str1 += "{:0>16}".format(inv.document_number.replace('-',''))
                        str1 += inv.date_invoice[8:10] + '/' + inv.date_invoice[5:7] + '/' + inv.date_invoice[0:4]
                        ...
                        t1 = sum(inv.tax_line_ids.filtered(lambda r: (
                                ...
                                r.tax_id.id in list(self.percep_code._ids)).mapped('base'))
                        t1 = MultiplybyRate(inv.currency_rate, t1, inv.company_currency_id, inv.currency_id)
                        str1 += "{:0>16}".format(str(t1).replace('.',','))
                        ...
                        str1 += "{:>11}".format(" ")
                    str1 += '\r\n'
                    if not date in adict.keys():
                        adict.update({date:{}})
                    if not inv.document_number in adict[date].keys():
                        adict[date].update({inv.document_number: []})
                    adict[date][inv.document_number].append(str1)
                    str1 = ""
        return adict

    def arciba_to_txt(self):
        if self.do_percep and not list(self.percep_code._ids):
            raise Warning(_('Debe Completar al menos un codigo de Percepcion'))
        if self.do_wh and not list(self.wh_code._ids):
        ...
        if self.do_percep:
            ...
            invoices = invoices.search(domain,order="date_invoice")
            adict1 = self.do_percepf(invoices,nc=False)
        ...
        if self.do_wh:
            ...
            domain = [
                ...
                ('tax_withholding_id','in',self.wh_code._ids),
                ('state','!=','draft')]
            ...
            for pay in payments:
                ...
                if pay.vendorbill.document_type_id.internal_type == 'debit_note' and pay.vendorbill.mipymesf:
                    type = "11"
                elif pay.vendorbill.document_type_id.internal_type == 'invoice' and pay.vendorbill.mipymesf:
                    type = "10"
                elif pay.vendorbill.document_type_id.internal_type == 'invoice':
                    type = "01"
                ...
                str1 += "{:0>16}".format(pay.vendorbill.document_number.replace('-',''))
                str1 += pay.vendorbill.date_invoice[8:10] + '/' + pay.vendorbill.date_invoice[5:7] + '/' + pay.vendorbill.date_invoice[0:4]

                t1 = pay.vendorbill.amount_total
                # t1 = pay.vendorbill.amount_untaxed + sum(pay.vendorbill.tax_line_ids.filtered(
                #     lambda r: (r.tax_id.tax_group_id.type == 'tax')).mapped('amount'))
                t1 = MultiplybyRate(pay.vendorbill.currency_rate, t1, pay.vendorbill.company_currency_id, pay.currency_id)
                str1 += "{:0>16}".format(str(t1).replace('.',','))

                str1 += "{:>16}".format(pay.withholding_number.replace('/','').replace('-',''))
                ...
                t1 = MultiplybyRate(pay.vendorbill.currency_rate, pay.withholdable_invoiced_amount, pay.vendorbill.company_currency_id,pay.currency_id)
                str1 += "{:0>16}".format(str(t1).replace('.',','))

                str1 += "{:0>5}".format(str("{:.2f}".format(pay.wh_perc)).replace('.',','))

                t1 = MultiplybyRate(pay.vendorbill.currency_rate, pay.amount, pay.vendorbill.company_currency_id,pay.currency_id)
                str1 += "{:0>16}".format(str(t1).replace('.',','))
                str1 += "{:0>16}".format(str(t1).replace('.',','))
                if not pay.vendorbill.mipymesf:
                    str1 += " "
                    str1 += "{:>10}".format(" ")
                else:
                    if not pay.vendorbill.acceptance:
                        str1 += " "
                        str1 += "{:>10}".format(" ")
                    else:
                        str1 += pay.vendorbill.acceptance
                        datetmp = ""
                        if pay.vendorbill.acceptance == "E":
                            datetmp = datetime.strptime(pay.vendorbill.acceptance_date, "%Y-%m-%d").strftime("%d/%m/%Y")
                        str1 += "{:>10}".format(datetmp)
                str1 += '\r\n'
                if not date in adict1.keys():
                    adict1.update({date:{}})
                if not pay.vendorbill.document_number in adict1[date].keys():
                    adict1[date].update({pay.vendorbill.document_number:[]})
                adict1[date][pay.vendorbill.document_number].append(str1)
                str1 = ""
        #file2
        if self.do_percep:
            invoices = self.env['account.invoice']
            ...
            invoices = invoices.search(domain,order="date_invoice")
            adict2 = self.do_percepf(invoices,nc=True)

        #SORT
        str1 = ""
        str2 = ""
        for l in sorted(adict1):
            for i in adict1[l]:
                for x in adict1[l][i]:
                    str1 += x
        for l in sorted(adict2):
            for i in adict2[l]:
                for x in adict2[l][i]:
                    str2 += x
