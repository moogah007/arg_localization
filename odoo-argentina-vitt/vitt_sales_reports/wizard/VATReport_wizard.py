...
class sales_reports(models.TransientModel):
    ...
    def Print_to_excel(self):
        ...
        if self.det_level == 'detailed':
            ...
            subindex += 1
            worksheet.write(index,subindex,_('IIBB State'))
            subindex += 1
        ...
        for o in invoices:
            if o.journal_id.use_documents and o.validated_inv(self):
                ...
                if self.det_level == 'detailed':
                    ...
                    if o.region_tax_control_id.name:
                        state = o.region_tax_control_id.name
                    else:
                        if o.state_id:
                    ...
                    if state:
                        worksheet.write(index, subindex, state)
                    subindex += 1
...
class def_sales_reports_reportvat(models.Model):
    ...
    @api.multi
    def gettotpercepincome(self,invoice=None):
        if invoice != None:
            self = invoice
        self.ensure_one()
        totval = sum(self.tax_line_ids.filtered(lambda r: (
            r.tax_id.tax_group_id.type == 'perception' and
            r.tax_id.tax_group_id.tax in ('vat','profits') and
            r.tax_id.vatreport_included == True)).mapped('amount'))
        total = float(MultiplybyRate(self.currency_rate, totval,self.company_currency_id,self.currency_id))
        if self.document_type_id.internal_type == 'credit_note':
            total *= -1
    ...
    @api.multi
    def gettotlocPercep(self,invoice=None):
        if invoice != None:
            self = invoice
        self.ensure_one()
        totval = sum(self.tax_line_ids.filtered(lambda r: (
            r.tax_id.tax_group_id.type == 'perception' and
            r.tax_id.tax_group_id.tax in ('vat','gross_income','profits') and
            r.tax_id.tax_group_id.application == 'municipal_taxes' and
            r.tax_id.vatreport_included == True)).mapped('amount'))
        total = float(MultiplybyRate(self.currency_rate, totval,self.company_currency_id,self.currency_id))
        if self.document_type_id.internal_type == 'credit_note':
            total *= -1
    ...
    @api.multi
    def gettotothers(self,invoice=None):
        if invoice != None:
            self = invoice
        self.ensure_one()
        totval = sum(self.tax_line_ids.filtered(lambda r: (
            ...
            r.tax_id.tax_group_id.application == 'others' and
            r.tax_id.vatreport_included == True)).mapped('amount'))
        total = float(MultiplybyRate(self.currency_rate, totval,self.company_currency_id,self.currency_id))
        if self.document_type_id.internal_type == 'credit_note':
            total *= -1
    ...
    @api.multi
    def gettotottaxrefund(self,invoice=None):
        if invoice != None:
            self = invoice
        self.ensure_one()
        totval = sum(self.tax_line_ids.filtered(lambda r: (
            r.tax_id.tax_group_id.type == 'tax_refund' and
            r.tax_id.tax_group_id.tax == 'vat' and
            r.tax_id.tax_group_id.application == 'national_taxes' and
            r.tax_id.vatreport_included == True)).mapped('amount'))
        totval = -totval
        total = float(MultiplybyRate(self.currency_rate, totval,self.company_currency_id,self.currency_id))
        if self.document_type_id.internal_type == 'credit_note':
    ...
    @api.multi
    def getotherNP(self, invoice=None):
        if invoice != None:
            self = invoice
        self.ensure_one()
        totval = sum(self.tax_line_ids.filtered(lambda r: (
                r.tax_id.tax_group_id.type == 'perception' and
                r.tax_id.tax_group_id.tax in ['profits','other'] and
                r.tax_id.tax_group_id.application == 'national_taxes' and
                r.tax_id.vatreport_included == True)).mapped('amount'))
        total = float(MultiplybyRate(self.currency_rate, totval, self.company_currency_id, self.currency_id))
        if self.document_type_id.internal_type == 'credit_note':
    ...
    @api.multi
    def gettotgrossincome2(self,invoice=None):
        if invoice != None:
            self = invoice
        self.ensure_one()
        totval = sum(self.tax_line_ids.filtered(lambda r: (
    ...
    @api.multi
    def gettotprofit(self,invoice=None):
        if invoice != None:
            self = invoice
        self.ensure_one()
        totval = sum(self.tax_line_ids.filtered(lambda r: (
            r.tax_id.tax_group_id.type == 'perception' and
            r.tax_id.tax_group_id.tax == 'profits' and
            r.tax_id.vatreport_included == True)).mapped('amount'))
        total = float(MultiplybyRate(self.currency_rate, totval,self.company_currency_id,self.currency_id))
        if self.document_type_id.internal_type == 'credit_note':
            total *= -1
    ...
    def gettotinttaxes3(self,invoice=None):
        if invoice != None:
            self = invoice
        self.ensure_one()
        totval = sum(self.tax_line_ids.filtered(lambda r: (
