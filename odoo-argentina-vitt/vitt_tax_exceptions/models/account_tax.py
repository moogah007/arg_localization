...
class ResCompany(models.Model):
    ...
    def get_arba_login_url(self, environment_type):
        ...
        #         '/SeguridadCliente/dfeServicioConsulta.do')
        #return arba_login_urlelse:
        if environment_type == 'production':
            env = 'prod'
        else:
            env = 'dev'
        set = self.env['ar.tax.servers'].search([
            ('ws_type', '=', 'ws_arba'),
            ('type', '=', 'primary'),
            ('enviroment', '=', env),
        ],limit=1)

        return set.url_server
    ...
    def get_arba_data(self, partner, from_date, to_date):
        ...
        _logger.info('Getting ARBA data for cuit %s from date %s to date %s' % (cuit, from_date, to_date))
        ws = self.arba_connect()
        ws.ConsultarContribuyentes(from_date,to_date,cuit)

        if (ws.Excepcion and ws.CodigoError != '11') or ws.CodigoError:
            set = self.env['ar.tax.servers'].search([('ws_type', '=', 'ws_arba'), ('type', '=', 'secondary')], limit=1)

            server = set.url_server
            ...
            try:
                ...
                raise Warning(_('Error conectandose con el servicio de ARBA Nuestro, comuniquese con el soporte de Moogah'))

            uid = common.authenticate(database, user, pwd, {})
            if not uid:
                raise Warning(_('Error user/pass erroneo para servidor ARBA Nuestro, comuniquese con el soporte de Moogah'))

            try:
                OdooApi = xmlrpclib.ServerProxy('%s/xmlrpc/2/object' % server)
                filters = [[['main_id_number', '=', partner.main_id_number],['sdate','<=',str(from_date)],['edate','>=',str(to_date)]]]
                padron = OdooApi.execute_kw(database, uid, pwd, 'ret.padron.arba', 'search_read', filters)
            except Exception as e:
                raise Warning(_('Error, usuario sin acceso a los padrones de ARBA Nuestro, comuniquese con el soporte de Moogah'))
            ...
            if padron:
                ...
                _logger.info('We get the following data from our ARBA server: \n%s' % data)
            else:
                data = {
                    'numero_comprobante': 'SIN DATOS',
                    'codigo_hash': '',
    ...
    def get_agip_data(self,partner,sdate,edate,apg=False):
        ...
        set = self.env['ar.tax.servers'].search([('ws_type', '=', 'ws_agip'),('type', '=', 'primary')],limit=1)
...
class ArTaxServers(models.Model):
    _name = 'ar.tax.servers'

    ws_type = fields.Selection([('ws_arba','WS ARBA'),('ws_agip','WS AGIP')],string="WS Tax Type",translate=True)
    url_server = fields.Char(string="URL Server",translate=True)
    type = fields.Selection([
        ('primary','Primary'),('secondary','Secondary'),('inactive','Inactivo')],string="Server Type",translate=True)
    database = fields.Char(string="Data Base",translate=True)
    enviroment  = fields.Selection([('dev','Test'),('prod','Produccion')],string="Entorno",
        default="dev",translate=True,required=True)