...
class AfipWsConsultWizard(models.TransientModel):
    ...
    def confirm(self):
        ...
        if inv.journal_id.afip_ws == 'wsfex':
            ...
            if 'FEXResultGet' not in doc['soap:Envelope']['soap:Body']['FEXGetCMPResponse']['FEXGetCMPResult'].keys():
                raise UserError(_('No existen datos de esa factura en la Afip'))

            res = doc['soap:Envelope']['soap:Body']['FEXGetCMPResponse']['FEXGetCMPResult']['FEXResultGet']
            if not 'Cae' in res.keys() or not 'Fch_venc_Cae' in res.keys():
            ...
            if date.replace("-", "")==res['Fecha_cbte'].replace("-", "") and \
                ...
                int(res['Cbte_tipo'])==int(inv.journal_document_type_id.document_type_id.code) and \
                res['Moneda_Id']==inv.currency_id.afip_code:
