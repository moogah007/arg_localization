...
class PercepTaxReportWizard(models.TransientModel):
    ...
    def doit(self):
        ...
        invoices = self.env['account.invoice'].search(domain,order='date')
        if invoices:
            index = 0
            for inv in invoices:
                ...
                if self.show_comp_cur:
                    cur1 = inv.company_id.currency_id.symbol
                else:
                    cur1 = inv.currency_id.symbol
                cur2 = inv.currency_id.symbol
                ...
                for tax in inv.tax_line_ids:
                    ...
                    if doit:
                        ...
                        if self.type != 'purchase':
                            lines.update({key: [
                                str(datetime.strptime(inv.date, "%Y-%m-%d").strftime("%d-%m-%Y")),
                                inv.display_name2,
                                inv.partner_id.name,
                                inv.partner_id.main_id_number,

                                inv.partner_id.afip_responsability_type_id.name,
                                inv.partner_id.gross_income_type,
                                inv.partner_id.gross_income_number,
                                inv.amount_total,

                                tax.base*sign,
                                base,
                                tax.tax_id.jurisdiction_code.name,
                                tax.amount*sign,
                                tax.tax_id.name,
                                #(inv.amount_total - inv.amount_untaxed - tax.amount) * sign,
                                cur1,
                                cur2,
                            ]})
                        else:
                            ...
                            if not testf:
                                if self.type != 'purchase':
                                    if self.print_by != 'excel':
                                        ...
                                        grouped.update({key:[tt,"","","","","","",0,0,0,"",0,"","",0,"","",""]})
                                else:
                                    if self.print_by != 'excel':
                    ...
                    if list_align[index] != 'none':
                        if list_align[index] != 'link':
                            if list_align[index] == 'center':
                                if print_data[data][index] != False:
