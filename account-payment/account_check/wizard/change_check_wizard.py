...
class account_change_check_wizard(models.TransientModel):
    ...
    def change(self):
        ...
        #if datetime.strptime(str(self.payment_date)[:10],"%Y-%m-%d").date() < \
        #        fields.Datetime.from_string(fields.Datetime.now()).date():
        #    raise UserError(_("Fecha Pago no puede ser menor a fecha del Dia"))
...
class AccountChangeCheckWizard(models.TransientModel):
    _name = 'account.change.check.third.wizard'
    _inherit = 'account.change.check.wizard'

    @api.model
    def get_original_check(self):
        return self.original_check_third_id.browse(self._context.get('active_id'))

    original_check_third_id = fields.Many2one(
        'account.check.third',
        'Original Check',
        required=True,
        default=get_original_check,
    ...
    @api.onchange('original_check_third_id')
    def change_original_check(self):
        self.checkbook_id = self.original_check_third_id.checkbook_id
        self.owner_vat = self.original_check_third_id.owner_vat
        self.owner_name = self.original_check_third_id.owner_name.id
        #self.bank_id = self.journal_id.bank_id

    @api.multi
    def change(self):
        self.ensure_one()
        number = self.original_check_third_id.nextCheckNumber(self.number)
        #if datetime.strptime(str(self.payment_date)[:10],"%Y-%m-%d").date() < \
        #        fields.Datetime.from_string(fields.Datetime.now()).date():
        #    raise UserError(_("Fecha Pago no puede ser menor a fecha del Dia"))
        ...
        if number != "0":
            vals = {
                ...
                'number': number,  # -------------------------- wizard
                'checkbook_id': self.checkbook_id.id,  # -------------------------- wizard
                'type': self.original_check_third_id.type,
                'partner_id': self.original_check_third_id.partner_id.id,
                # 'state': self.original_check_id.state,
                'state': 'draft',
                'issue_date': self.issue_date,  # --------------------------- wizard
                'owner_vat': self.original_check_third_id.owner_vat,
                'owner_name': self.original_check_third_id.owner_name.id,
                'bank_id': self.bank_id.id,  # --------------------------- wizard
                'amount': self.original_check_third_id.amount,
                'amount_currency': self.original_check_third_id.amount_currency,
                'currency_id': self.original_check_third_id.currency_id.id,
                'payment_date': self.payment_date,  # ---------------------------- wizard
                'journal_id': self.original_check_third_id.journal_id.id,
                'company_id': self.original_check_third_id.company_id.id,
                'company_currency_id': self.original_check_third_id.company_currency_id.id
            }
            if self.original_check_third_id.type == 'issue_check':
                vals.update({'bank_id': self.checkbook_id.debit_journal_id.bank_id.id})
            new_check_mod = self.env['account.check.third']
            new_check = new_check_mod.create(vals)

            paym = self.env['account.payment'].search([('check_third_id', '=', self.original_check_third_id.id)])
            if paym:
                paym.write({'check_third_id': new_check.id})
            new_check.write({'state': self.original_check_third_id.state})

            # new_check = self.original_check_id.sudo().copy(vals)
            self.original_check_third_id.sudo().write({
                # 'replacing_check_id': new_check.id,
                'amount': 0.0,
                'company_currency_amount': 0.0,
            })

            self.original_check_third_id._add_operation('changed', new_check)
            new_check._add_operation('holding', self.original_check_third_id)
