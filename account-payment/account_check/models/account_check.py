...
# ex campos related
    ...
    new_third_id = fields.Many2one('account.check.third',string="New Check",translate=True)

    def copy_checks(self):
        checks = self.env['account.check'].search([('state', '=', 'holding'),('type', '=', 'third_check')])
        for check in checks:
            vals = {
                'old_third_id': check.id,
                'name': check.name,
                'no_order': check.no_order,
                'payment_date': check.payment_date,
                'issue_date': check.issue_date,
                'owner_name': check.owner_name.id,
                'checkbook_id': check.checkbook_id.id,
                'amount_cash': check.amount_cash,
                'notes': check.notes,
                'cashin_out_id': check.cashin_out_id.id,
                'message_last_post': check.message_last_post,
            ...
            newcheck = self.env['account.check.third'].create(vals)
            check.write({'new_check_id': newcheck.id})
            for oper in check.operation_ids:
                newcheck.operation_third_ids.create({
                    'date': oper.date,
                    'operation': oper.operation,
                    'origin_name': oper.origin_name,
                    'partner_id': oper.partner_id.id,
                    'notes': oper.notes,
                    'check_third_id': newcheck.id
                })
                payments = self.env['account.payment'].search([('check_ids', '=', check.id)])
                for pay in payments:
                    pay.write({'check_third_id': newcheck.id, 'check_id': False})
    ...
    def create(self, vals):
        #print vals
        if vals.get('type') != 'third_check':
            vals['type'] = 'issue_check'
...
#            self._add_operation('returned', move)
    ...
    def action_create_debit_note(self, operation, partner_type, partner, account, amount, account_company=None,credit=False):
        ...
        if self.type == 'third_check':
            ...
            if last_operation in ['returned','reclaimed','debited']:
        ...
        if operation == 'debited' and self.type != 'third_check':
            self._add_operation('reclaimed', invoice, partner, None)
        else:
...
class AccountCheckThird(models.Model):
    _name = 'account.check.third'
    _inherit = 'account.check'

    operation_third_ids = fields.One2many(
        'account.check.third.operation',
        'check_third_id',
    )
    type = fields.Selection([('third_check', 'Third Check')],translate=True,default='third_check')
    old_third_id = fields.Many2one('account.check',string="Old Check",translate=True)
    ...
    def get_bank_vals(self, action, journal=None, date=None):
        ...
        if action == 'bank_debit':
            ...
            credit_account = journal.default_debit_account_id
            operation = self.operation_third_ids.search([('check_third_id', '=', self.id), ('operation', '=', 'deposited')],
                                                  limit=1)
            for line in operation.move_id.line_ids:
                if line.debit > 0:
                    credit_account = line.account_id
    ...
    @api.model
    def create(self, vals):
        vals['type'] = 'third_check'
        print vals
        res = super(AccountCheckThird, self).create(vals)
        print res.state
        res._check_unique()
        return res
    ...
    @api.multi
    @api.depends('operation_third_ids.operation','operation_third_ids.date')
    def _compute_state(self):
        for rec in self:
            if rec.operation_third_ids:
                operation = rec.operation_third_ids[0].operation
                rec.state = operation
            else:
                rec.state = 'draft'
    ...
    def _add_operation(self, operation, origin, partner=None, date=False, moveid=None):
        for rec in self:
            ...
            rec.operation_third_ids.create(vals)

    def _get_last_operation_name(self, level=1): # Get last Operation
        for rec in self:
            try:
                rec.prev_state = rec.operation_third_ids[level].operation
            except:
                rec.prev_state = "No Prev State"
    ...
    def _get_operation(self, operation, partner_required=False, model=''):
        self.ensure_one()
        if model:
            operation = self.operation_third_ids.search(
                [('check_third_id', '=', self.id), ('operation', '=', operation),('owner_model','=', model)], limit=1)
        else:
            operation = self.operation_third_ids.search(
                [('check_third_id', '=', self.id), ('operation', '=', operation)], limit=1)
        if partner_required:
            if not operation.partner_id:
                raise ValidationError((
                    'The %s operation has no partner linked.'
    ...
    @api.multi
    def _get_last_operation(self, level=-2): # Get last Operation
        return self.operation_third_ids[level]
...
class AccountCheckOperation(models.Model):
    ...
    _name = 'account.check.third.operation'
    _rec_name = 'operation'
    _order = 'date desc, id desc'

    check_third_id = fields.Many2one(
        'account.check.third',
        'Check',
        required=True,
        ondelete='cascade'
...
class AccountMoveLine(models.Model):
    ...
    check_third_id = fields.Many2one('account.check.third', related='payment_id.check_third_id', string='Check third', readonly=True)