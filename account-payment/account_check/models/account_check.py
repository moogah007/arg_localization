    ...
    def action_create_debit_note(self, operation, partner_type, partner, account, amount, account_company=None,credit=False):
        ...
        if partner_type == 'customer':
            if operation == 'reclaimed':
                product = self.env['product.template'].search([('tc_state','=','tc_rec_endorsed')]).id
            if operation == 'rejected':
                product = self.env['product.template'].search([('tc_state','=','tc_rejected')]).id
            if operation == 'returned':
                product = self.env['product.template'].search([('tc_state','=','tc_canceled')]).id
        else:
            if operation == 'rejected':
                product = self.env['product.template'].search([('tc_state','=','oc_rejected')]).id
            if operation == 'returned':
                product = self.env['product.template'].search([('tc_state','=','oc_canceled')]).id
        if not product:
