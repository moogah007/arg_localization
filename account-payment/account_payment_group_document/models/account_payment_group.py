...
class AccountPaymentGroup(models.Model):
    ...
    def _onchange_display_name(self):
        for rec in self:
            rec.display_name2 = rec.name

    @api.multi
    def _onchange_display_name_cron(self):
        apg = self.env['account.payment.group'].search([])
        for rec in apg:
            rec.write({'display_name2':rec.display_name})
    ...
    def write(self, values):
        values.update({'display_name2': self.name})
        return super(AccountPaymentGroup, self).write(values)
