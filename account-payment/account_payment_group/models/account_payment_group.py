...
class AccountPaymentGroup(models.Model):
    ...
    unmatched_amount2 = fields.Float(string="hidden support field")
    ...
    def create(self, values):
        ...
        if 'payment_date2' in values.keys():
            ...
            values['unmatched_amount2'] = self.unmatched_amount
    ...
    def write(self, vals):
        for rec in self:
            vals['unmatched_amount2'] = rec.unmatched_amount
...
class AccountPaymentGroupConfirm(models.TransientModel):
    ...
    This wizard will confirm the all the selected draft invoices
    """
    _name = "account.payment.group.maint"
    _description = "Confirm the selected Payments"
    ...
    def payment_confirm(self):
        ...
        for record in self.env['account.payment.group'].browse(active_ids):
            record.unmatched_amount2 = record.unmatched_amount
        return {'type': 'ir.actions.act_window_close'}
