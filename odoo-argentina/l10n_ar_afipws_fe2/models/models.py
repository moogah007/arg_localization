...
class AccountInvoice(models.Model):
    ...
    def get_related_invoices_data(self):
        ...
        self.ensure_one()
        if self.origin:
            rel_invoices = self.search([('display_name2', '=', self.origin)])
            return rel_invoices
        return False
    ...
    def do_pyafipws_request_cae(self):
        ...
        for inv in self:
            ...
            if afip_ws not in ('wsfe','wsmtxca','wsfex','wsbfe'):
                continue
            ...
            if inv.mipymesf:
                if not inv.date_due:
                    raise UserError(_('You should complete due date'))
            ...
            if inv.mipymesf:
                if not doc_afip_code in ('201','206','211'):
                    fecha_venc_pago = ''
                else:
            ...
            fecha_pago = ""
            if (int(concepto) == 2 or int(concepto) == 3) and doc_afip_code == '19':
            ...
            if (afip_ws == 'wsfe' or afip_ws == 'wsbfe') and inv.mipymesf:
            ...
            if CbteAsoc:
                if inv.mipymesf:
                    cuit = inv.company_id.main_id_number
                    fcha = CbteAsoc.date_invoice.replace('-','')
                    ws.AgregarCmpAsoc(
            ...
            if afip_ws != 'wsfe':
                for line in inv.invoice_line_ids:
                    ...
                    bonif = str("%.2f" % abs((precio*qty)*line.discount/100)) or None
                    if afip_ws == 'wsbfe':
                        if line.product_id.mercosur_code:
                            ...
                            raise Warning(_('Please complete Mercosur Code in product %s' % (u''.join(line.product_id.name))))
                    ...
                    if afip_ws == 'wsmtxca':
                        ...
                        ws.AgregarItem(
                            u_mtx, cod_mtx, codigo, remove_accents(ds), qty, umed,
                            precio, bonif, iva_id, imp_iva, importe + imp_iva)
                    elif afip_ws == 'wsfex':
                        ws.AgregarItem(
                            codigo, remove_accents(ds), qty, umed, precio, importe,
                            bonif)
                    elif afip_ws == 'wsbfe':
                        for tribute in line.product_id.taxes_id:
                            if tribute.tax_group_id.tax == 'vat':
                                ...
                                break
                        ws.AgregarItem(
                            pro_codigo_ncm, pro_codigo_sec, remove_accents(ds), qty, umed, precio, bonif,
                            iva_id, importe)
            ...
            if msg:
                ...
                raise UserError(_('AFIP Validation Error. %s, \n\n\n Sent: %s' % (msg, ws.XmlRequest)))
...
class AccountInvoiceRefund(models.TransientModel):
    ...
    def invoice_refund(self):
        ...
        nc.write({
            'mipymesf': nc.journal_document_type_id.document_type_id.mipymesf,
            'mipymesf_read_only': nc.journal_document_type_id.document_type_id.mipymesf,
            'comercial_ref': invoice_id.comercial_ref,
            'cbu_alias': invoice_id.cbu_alias,
            'cbu': invoice_id.cbu,
            'bank_account_id': invoice_id.bank_account_id.id,
        })
        return res
