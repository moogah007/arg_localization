...
# impuestos e importes de impuestos
    ...
    def get_localization_invoice_vals(self):
        for rec in self:
            if rec.localization == 'argentina':
                commercial_partner = rec.partner_id.commercial_partner_id
                currency_rate = rec.currency_id.compute(1., rec.company_id.currency_id)
                return {
                    'afip_responsability_type_id': (commercial_partner.afip_responsability_type_id.id),
    ...
    def _get_available_journal_document_types(
        ...
        * Documents configuration
        ...
        if invoice_type in ['out_invoice', 'out_refund', 'in_refund']:
