...
class AccountInvoice(models.Model):
    ...
    * we add display_name to replace name field use, we add
    ...
    acceptance = fields.Selection([('E','Express Acceptance'),('T',' Tacit Acceptance')],
                                  string="acceptance",
                                  readonly=True,
    ...
    acceptance_date = fields.Date(string="Express Acceptance Date")
    cancel_date = fields.Date(string="Cancel Date",translate=True)
    ...
    def get_available_journal_document_types(self):
        ...
        """
        for invoice in self:
            invoice.available_journal_document_type_ids = False
            invoice.journal_document_type_id = False
    ...
    def create(self, vals):
        ...
        if vals.get('acceptance'):
            if vals['acceptance'] not in ('E','T'):
                vals.pop('acceptance')
    ...
    def write(self, vals):
        for rec in self:
            vals.update({
                'mipymesf': rec.journal_document_type_id.document_type_id.mipymesf,
                'mipymesf_read_only': rec.journal_document_type_id.document_type_id.mipymesf,
            })
            if 'state' in vals.keys():
                if vals['state'] == 'cancel':
                    vals.update({'cancel_date': datetime.now().strftime('%Y-%m-%d')})
