    ...
    def validate_deposit(self):
        ...
        for deposit in self:
            ...
            for line in deposit.check_payment_ids:
                ...
                line.check_third_id._add_operation('deposited', self, None, None, move.id)
            ...
            #[reconcile_lines.reconcile() for reconcile_lines in to_reconcile_lines]
...
class AccountMoveLine(models.Model):
    ...
    check_id = fields.Many2one('account.check', related='payment_id.check_id', string='Check', readonly=True)
    check_state = fields.Selection(related='check_third_id.state', string='Check State', readonly=True)
    bank_id = fields.Many2one('res.bank', related='check_third_id.bank_id', string='Bank', readonly=True)
    check_issue_date = fields.Date(related='check_third_id.issue_date', string='Issue Date', readonly=True)
    check_deposit_id = fields.Many2one('account.check.deposit', string='Check Deposit', copy=False)
    check_amount = fields.Monetary(related='check_third_id.amount', string='Amount', readonly=True)
    check_payment_date = fields.Date(related='check_third_id.payment_date', string='Payment Date', readonly=True)
    number = fields.Char(related='payment_id.check_id.number', string='Check', readonly=True)
