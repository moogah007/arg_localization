...
# on change methods
    ...
    # @api.constrains('check_ids')
    @api.onchange('check_third_id', 'payment_method_code')
    def onchange_checks_third(self):
        # we only overwrite if payment method is delivered
        if self.payment_method_code == 'delivered_third_check':
            self.amount = sum(self.check_third_id.mapped('amount'))
    ...
    @api.depends('checkbook_id','check_name2')
    @api.onchange('check_name2')
    @api.constrains('check_name2')
    def onchange_check_name2(self):
        if self.check_name2:
            if not ((self.partner_id == self.check_name2.partner_id or self.check_name2.partner_id.id==False) \
                    and self.check_name2.state == 'draft' and self.checkbook_id == self.check_name2.checkbook_id):

                raise UserError(_('the check should be with the correct partner/checkbook/state'))
            if not (self.check_name2.amount > 0):
                raise UserError(_('the amount in check should be greater than 0'))

            if not self.check_name2.partner_id.id:
                raise UserError(_('the partner in check shouldn\'t be empty'))

            self.amount = self.check_name2.amount
            self.check_number = self.check_name2.number
            self.check_issue_date = self.check_name2.issue_date
            self.check_payment_date = self.check_name2.payment_date
            if self.payment_method_code != 'issue_check':
                self.check_bank_id = self.check_name2.bank_id
    ...
    def create_check(self, check_type, operation, bank, origin=None, move_id=None):
        ...
        if not self.check_name:
            ...
            if check_type == 'third_check':
                ...
                print 1.2
                check = self.env['account.check.third'].create(check_vals)
            else:
                print 1.3
        ...
        if check_type != 'third_check':
            ...
            self.check_third_id = check.id

        if move_id:
            move = self.env['account.move'].search([('id','=',move_id)])
            for line in move.line_ids:
                if check_type == 'third_check':
                    line.write({'check_third_id':check.id})
                else:
        ...
        * from _get_liquidity_move_line_vals to add check operation and, if
        ...
        if (
            ...
            if cancel:
                ...
                rec.check_third_id._del_operation(operation)
                rec.check_third_id = False
                return None
            ...
            rec.check_third_id._add_operation(operation, rec.payment_group_id, False, False, self.move_line_ids[0].move_id.id)
            vals['account_id'] = self.get_third_check_account().id
        elif (
                rec.payment_method_code == 'delivered_third_check' and
                rec.payment_type == 'transfer' and
            ...
            rec.check_third_id._add_operation(operation, rec.payment_group_id, rec.partner_id, False, self.move_line_ids[0].move_id.id)
            vals['account_id'] = self.get_third_check_account().id
        elif (
