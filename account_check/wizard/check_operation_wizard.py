...
#        readonly=True
    ...
    chk_type = fields.Selection([('third','third'),('issue','issue')])
    ...
    def action_confirm(self):
        ...
        print self.chk_type
        if self.chk_type == 'third':
            for check in self.env['account.check.third'].browse(self._context.get('active_ids', [])):
                if self.action_type == 'bank_reject':
                    ...
                    self.returned_t(check, self.date)
    ...
    @api.multi
    def returned_t(self,check, date):
        self.ensure_one()
    ...
    def claim(self, check, date, account=None, amount=None, exp_type=None):
        ...
        if check.type == 'third_check':
            ...
            if last_operation == 'debited':
                account_company = self.company_id._get_check_account('own_check_cancelled')
            else:
        ...
        if last_operation == 'debited':
            return check.action_create_debit_note('debited', partner_type, check.partner_id, account, amount,
                                                  account_company)
        else:
            return check.action_create_debit_note('reclaimed', partner_type, check.partner_id, account, amount,
                                                  account_company)
